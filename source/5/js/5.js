...
function PageTopCheck() {
  ...
  let secondTop = $("#sort").offset().top - 150; //#sortの上から150pxの位置まで来たら
...
$('.scroll-top a').click(function () {
  ...
  if (elmHash == "#gallery") { //もし、リンク先のhref の後が#galleryの場合
    ...
    $('body,html').animate({
      scrollTop: 0
    }, 500); //それ以外はトップへスクロール。数字が大きくなるほどゆっくりスクロール
  }
...
if (windowwidth > 768) {
  responsiveImage = [ //PC用の動画と画像
    {
      src: './img/main01.jpg', //動画が再生されなかった場合の代替画像
      video: {
        src: [ //mp4で動画が再生されない時のことを考えて複数の形式の動画を設定
          './video/movie.mp4', './video/movie.webm', './video/movie.ogv'
        ],
